...
#include"passenger.h"
#include"window.h"
 int NumOfWin;		//1.安检口总数目
 int MinCheck;		//2.最少开放安检口数量
 int MaxCustCheck;	//3.安检口队伍最大长度
 int MinRestSec;	//4.安检口最小休息时长
 int MaxRestSec;	//5.安检口最大休息时长
 int MaxSec;		//6.安检口最大安检时长 单位是秒
 int MinSec;		//7.安检口最小安检时长 单位是秒

 int MaxCustSingleLine;//8.缓冲区单队列最大乘客数
 int MaxLines;		//9.蛇形缓冲区最多由MaxLines个直队组成
 int MaxSeqLen;		//10.最大允许等待长度
 int EasySeqLen;	//11.短期等待长度
